...
### 008 复测暴露的三个缺陷与修复（2026-09-16，核心版本 2026-09-16h）

用户在 `范例/008`（Elisabeth-Tarot-柄.psd，2598×4724@1200，133 层）复测时报告「BG 组矢量蒙版错误」和「极光纹区域中央大片圆形空白」。逐条定位并修复：

1. **长中文正则漏匹配，`不做折光` 标记失效**。ExtendScript（PS 2020 实测）对长中文交替正则不可靠：旧 `NO_REFRACTION_RE`（20 项、117 字符）对 `hot stamp/Silver（不做折光）` 判 **false**，同一串用短正则 `/不做折光/` 判 true；把整条正则截到 108 字符（去掉末项 `\bskip\b`）又变 true。后果是「不做折光」组没被隐藏，还照常占用区域占用通道——BG 这类背景层的蒙版被 Silver/Queen 的区域整片挖掉，看起来就是「蒙版错误」。现中文标记改为 `indexOf` 逐项查找、英文标记保留短正则（`ZG.__selftest.isNoRefraction` 自检 17/17 通过）。
