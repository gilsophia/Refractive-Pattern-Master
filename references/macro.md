...
| `content_flow` | 随形流场 | 实现（沿画面分析出的 8x8 角度场积分流线，分段走势；中心距与 flow 对齐、只在空区起笔；无场时退回 flow） | `vortex` | 涡旋流场 | 实现（双臂螺旋近似） |
...
- **大层生成/裁剪加速（beta0.2b / 核心 2026-09-18a）**：区域裁剪改为"细网格奇偶栅格"（区域线段按 DDA 写格 → 扫描线求格心奇偶 → 查询点用"格心奇偶 + 与格内线段的交叉数"，与逐点 even-odd 数学等价；多边形带生成时算好的包围盒 `_bb`），生成阶段还用同一网格跳过完全在区域外的元素（这些元素本来就会被裁剪丢弃，输出一致）。实测：合成 A/B 140 秒→1.4 秒；010 的 Velvet_S 生成 108.6→43.0 秒、裁剪 27.0→2.8 秒；011 整份（5 层 @1200ppi）6 分 19 秒、5 组 0 失败。ExtendScript 的数组操作开销随规模超线性增长是"大层慢"的根因，后续进一步提速需要把多边形改成扁平数字数组（牵动全部生成器，留待 0.3）。
