...
GUI 中最小线宽固定 `0.10 mm`；隐藏层默认不生成（可手动勾选启用）。右侧「纹样预览」面板显示当前选中纹样的示意图（图来自 `texture-preview/` 的「中文名-键名.png」，调整参数不会实时重绘；缺图时给出提示但不影响生成）。勾选「读取画面内容」（默认开）后，「建议当前层 / 建议全部」会先分析图层画面再给建议。下拉提供全部已实现纹样（见下表），**菜单里显示中文名**（如 `折面平行纹`、`回纹`、`六角蜂巢纹`），图层列表行里写成「中文名(键名)」便于和 `job.json`、文档对照；内部仍按键名传参，中文名与键的对应取自 `design-logic.md` 的定稿命名。`高级参数` 框可用逗号分隔 `key=value` 传入如 `branch_angle_deg=30, petal_count=14, fan_arc_deg=90`。

### 智能建议（本地规则 + 候选集 + 相邻差分 + 可选画面分析）

GUI 内置**确定性启发式建议**：选中图层即显示一行「建议: …(原因)」，可用「建议当前层 / 建议全部」一键采纳。规则为：

1. 图层名关键词匹配（中英文，见 `refraction_core.jsx` 的 `SUGGEST_RULES` 表）：如「头发/hair→随形流场·flow、脸/face→跳过、边框/框/frame→diamond_tri、云/cloud→contour、水/water→wave、光环/halo→fan、羽/wing→feather、花/petal→petal_rosette、宝石/钻→facet·fan、金属→facet·parallel、火/焰→content_flow·flow、冰/雪→facet·triangle_lattice、木/竹→flow·topographic、沙/尘→dot_field、星空→dot_field、极光→moire_radial、文字/text→跳过」等。
2. **每条规则给的是候选集（按适合度排序），不是单一答案**；最终选哪个由上下文决定：优先避开相邻区块已用的纹样与家族（包围盒相邻即算相邻，地图涂色式差分），同类别内按全库用量轮换（每种纹样都有机会被用上），实在避不开时自动换 45° 方向。小区域先选、容器/背景最后补空，与生成的「子层优先」一致。
3. 图层名含「不加折光 / 不折光 / 不做 / 跳过 / skip」等 → 照常按原优先度生成、输出组生成后隐藏（见下节）。
4. 几何兜底（名字没命中时同样给候选集）：细长（长宽比>3）→ flow/content_flow/chevron…；占画布<3% 的小区域 → short_curve/dash_field/dot_field…；占画布>60% 的大面积 → parallel/wave/topographic/moire_radial…；否则 → parallel/facet/diamond_lattice/herringbone…。
5. **读取画面内容（勾选，默认开）**：点建议按钮时先用「中灰底合成 → 缩到 128px → BMP 解析 → 结构张量」分析画面（每层约 1–2 秒，结果按层缓存），得到主走向、方向一致度、弯折度、径向性（放射/同心）、细节密度与 8×8 角度场。名字没命中时按这些特征选候选（直纹走势/随形走势/放射状/同心状/细碎纹理/平缓面）；命中时用主走向修正 `direction_deg`。关闭勾选则只跑名称与几何规则（更快）。
...
| `pattern` | 纹样键，见下方纹样表；`content_flow` 无需额外配置（生成时自动分析该层画面取得走向场） |
...
| `content_flow` | 随形流场 | 实现（沿画面分析出的 8x8 角度场积分流线，分段走势；无场时退回 flow） | `vortex` | 涡旋流场 | 实现（双臂螺旋近似） |
| `wave` | 平行波纹 | 实现 | `petal_rosette` | 花瓣玫瑰纹 | 实现（默认 14 瓣） |
| `zigzag` | 连续折线纹 | 实现 | `diamond_lattice` | 菱形网格 | 实现（两组斜线） |
| `chevron` | 鱼骨/脊线纹 | 实现（脊线+斜支） | `diamond_tri` | 三角菱格纹 | 实现（正倒三角咬合成菱形，格内有细线） |
| `feather` | 羽片纹 | 实现（脊线+弯支） | `moire_radial` | 辐射摩尔纹 | 实现（三组微错位放射线干涉） |
| `herringbone` | 人字错列纹 | 实现 | `triangle_lattice` | 三角网格 | 实现（三组线） |
| `bilateral_flow` | 双向流线 | 实现（中缝留白） | `hex_lattice` | 六角蜂巢纹 | 实现 |
| `meander` | 回纹/迷宫纹 | 实现（较小单元） | `checker` | 棋盘方向纹 | 实现 |
| `contour` | 等距轮廓纹 | 实现 | `carbon_fiber` | 碳纤维纹 | 实现（密排错列短线） |
| `topographic` | 地形等高纹 | 实现（=contour） | `dash_field` | 错相短线场 | 实现 |
| `scale` | 鳞片纹 | 实现（密排弧形） | `dot_field` | 点阵/环点场 | 实现（密排环点） |
| `short_curve` | 稠密短曲线 | 实现（隔行错列） | | | |
...
图层或**编组**命名里带「不折光」类的字样时，该层及其全部子项仍然**照常生成**，但生成的输出组会被**隐藏**（`visible=false`），使母版与源文件的层级/可见性对应。

**标记只影响可见性，不影响生成逻辑**：这类层按**原优先度**参与「避免重复生成」——先扣除前面已占用的区域，再把自己占用的区域并入占用通道（后面的层不会再往这块铺纹样）。因此「不折光」区域在母版里保持**没有折光纹**，这也正是命名的本意；生成出来的那套纹样只作为与源层级对应的隐藏参考。
...
- 取样容差 `path_tolerance_px` 越小节点越多、越慢（默认 1 px）。大面积复杂图层可适当放大容差以缩短耗时。**大面积高分辨率图层（长边 >800px 且 ≥600ppi）在转路径前会自动做 1–2px 高斯模糊**，并把取样容差同步放宽到模糊半径，避免软渐变 alpha 的阈值轮廓过碎导致 `makeWorkPath` 长时间不返回（010 实测：1200ppi 的布料阴影层从卡住 17 分钟以上降到 2 秒）；区域蒙版因此只差零点几毫米（1200ppi 下 2px ≈ 0.04mm）。
...
- 本宏已在本机对多份真实 PSD 完整实测（007 / 008 / 001 / 010，含 87 层、1200ppi 的大文件），逐项结论见 `references/validation.md`；首次在自己的文件上运行仍建议先按 `references/validation.md` 的最小验证步骤试跑。
