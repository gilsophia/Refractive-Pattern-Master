...
| pattern | 名称 | 纹样键（界面下拉显示中文名），如 facet / flow / content_flow / contour / fan / diamond_tri / moire_radial / short_curve；`content_flow`（随形流场）会按该层画面走向生成流线，无需额外参数 |
...
```json
{
  ...
  "layers": [
    ...
    {"source_path": "人物/衣褶", "enabled": true, "pattern": "content_flow"},
    {"source_path": "人物/脸[不加折光]", "enabled": false},
    {"source_path": "背景", "enabled": true, "pattern": "fan", "inner_radius_mm": 8}
  ]
}
```
